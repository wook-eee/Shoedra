...
public class ItemRepositoryCustomImpl implements ItemRepositoryCustom{
    ...
    private BooleanExpression searchSellStatusEq(ItemSellStatus searchSellStatus){
        ...
        // ItemSellStatus null이면 리턴 null 아니면 SELL, SOLD 둘중 하나 리턴
    }
    ...
    private BooleanExpression regDtsAfter(String searchDateType){
        ...
        //dataTime을 시간에 맞게 세팅 후 시간에 맞는 등록된 상품이 조회하도록 조건값 반환
    }
    ...
    @Override
    public Page<Item> findAllWithRepImage(ItemSearchDto itemSearchDto, Pageable pageable){
        QItem item = QItem.item;
        QItemImg itemImg = QItemImg.itemImg;

        QueryResults<Item> results = queryFactory.selectFrom(item)
                .join(item.itemImgs, itemImg).fetchJoin()
                .where(
    ...
    public Page<MainItemDto> getMainItemPage(ItemSearchDto itemSearchDto, Pageable pageable){
        ...
        // QMainItemDto @QueryProjection을 허용하면 DTO 바로 조회 가능
